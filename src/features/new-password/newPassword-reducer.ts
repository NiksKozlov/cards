...
import { setRegisterServerError } from "../register/register-reducer";
...
export const newPasswordReducer = (state = initState, action: NewPasswordActionsType): InitStateType => {
  switch (action.type) {
...
export const createNewPasswordSuccess = (isCreateNewPassword: boolean): createNewPasswordSuccessActionType => ({
  type: "CREATE-PASSWORD-SUCCESS",
  isCreateNewPassword,
});
export const setNewPasswordServerError = (serverError: null | string): setNewPasswordServerErrorActionType => ({
  type: "SET-SERVER-ERROR",
  serverError,
});

export const createNewPassword = (password: string, someToken: string) => async (dispatch: AppThunkDispatch) => {
  try {
